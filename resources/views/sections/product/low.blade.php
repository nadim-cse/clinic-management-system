...
<h2 class="intro-y text-lg font-medium mt-10">
                   These Products Will Stock Out Soon
                </h2>
                <div class="grid grid-cols-12 gap-6 mt-5">
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
                        {{-- <button class="button text-white bg-theme-1 shadow-md mr-2">Add Notary</button> --}}
                        ...
                        <table class="table table-report -mt-2">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center whitespace-no-wrap">Current Stock</th>
                                    <th class="text-center whitespace-no-wrap">Alert Amount</th>
                            ...
                            <tbody>
                                ...
                                @php
                                    $index = 1;
                                @endphp
                                @foreach($stock_out_products as $stock_out_product)
                               
                                <tr class="intro-x">
                                    <td class="text-center"> @php echo $index ++ @endphp</td>
                                    <td class="text-center">{{ $stock_out_product->name}}</td>
                                    <td class="text-center">{{ $stock_out_product->power }}</td>
                                    <td class="text-center">{{ $stock_out_product->stock}}</td>
                                    <td class="text-center">{{ $stock_out_product->stock_out_alert}}</td>
                                    <td class="table-report__action w-56">
                                        <div class="flex justify-center items-center">
                                            {{-- <a class="flex items-center mr-3" href="{{ route('products.low.inventory', $stock_out_product->id) }}"> <i data-feather="eye" class="w-4 h-4 mr-1"></i> View </a> --}}
                                            <a class="flex items-center mr-3" href="javascript:;"> <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Update </a>
